...
def drawTitle(stdscr, y, x):
    # draw title
    stdscr.addstr(y, x, "    _             ___        ",
                  curses.color_pair(1))
    stdscr.addstr(y+1, x, "   /_\  _ _ _ __ / __|  _ _ _ ___ ___ ___",
                  curses.color_pair(1))
    stdscr.addstr(y+2, x, "  / _ \| '_| '_ \ (_| || | '_(_-</ -_|_-<",
                  curses.color_pair(1))
...
def makeTextBox(y, x, length=1):
    win = curses.newwin(1, length, y+1, x+1)  # y+1 & x+1 for fit whit drawBox
    win.bkgd(' ', curses.color_pair(1))
    return curses.textpad.Textbox(win)


def dialogBox(stdscr, y, x, msg, title="infobox"):
    curses.textpad.rectangle(stdscr, y, x, y+4, len(msg)+x+1)  # message box
    stdscr.addstr(y, x+1, title, curses.color_pair(1))  # set box title
    stdscr.addstr(y+1, x+1, (" "*(len(msg))), curses.color_pair(1))
...
def infoBox(stdscr, y, x, msg, title="infobox"):
    ...
    stdscr.addstr(y+3, (x*2+len(msg))/2, "Ok", curses.color_pair(1))
    stdscr.getkey()  # wait any key


def quitBox(stdscr, y, x, msg, title="quitbox"):
    dialogBox(stdscr, y, x, msg, title)
    stdscr.addstr(y+3, ((x*2+len(msg))/2)-3, "Quit    No",
                  curses.color_pair(1))
    stdscr.addstr(y+3, ((x*2+len(msg))/2)-3, "Q", curses.color_pair(2))
    stdscr.addstr(y+3, ((x*2+len(msg))/2)+5, "N", curses.color_pair(2))
    while 1:
...
def saveBox(stdscr, y, x, msg, title="savebox"):
    dialogBox(stdscr, y, x, msg, title)
    stdscr.addstr(y+3, ((x*2+len(msg))/2)-3, "Yes    No", curses.color_pair(1))
    stdscr.addstr(y+3, ((x*2+len(msg))/2)-3, "Y", curses.color_pair(2))
    stdscr.addstr(y+3, ((x*2+len(msg))/2)+4, "N", curses.color_pair(2))
    while 1:
        digit = stdscr.getkey()  # wait any key
        if digit == "y":
            return 1
...
def drawMenuBar(stdscr, length_win):
    ...
    stdscr.addstr(0, 0, "1.Attack    2.Scan    3.Sniff    4.Network    Q.Quit",
                  curses.color_pair(2))


def menuSelects(digit):
    if (digit == "1" or
        digit == "2" or
        digit == "3" or
        digit == "4" or
